 ...
 *  Last modification date 28/01/2013 01:04 AM.
 *
 * Description :
...
class cliprz_session_handler_files
{
    ...
    public function __construct()
    {
        ...
        if (is_dir($_config['session']['save_path']))
        {
            session_save_path($_config['session']['save_path']);
        }
        else
        {
            if (c_mkdir($_config['session']['save_path'],0777))
            {
                c_create_index(c_trim_path($_config['session']['save_path']).DS);
                session_save_path($_config['session']['save_path']);
            }
        }
    ...
    {
        $this->session_path = c_trim_path($session_path).DS;
        ...
        {
            trigger_error("Can not open [".$this->session_path."] session.");
        }
    }
