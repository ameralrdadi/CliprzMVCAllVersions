 ...
 *  Last modification date 08/12/2012 11:50 PM.
 *
 * Description :
 *  Sessions class.
 *
...
class cliprz_session
{
    ...
    protected static function session_gc_configuration ($gc)
    {
        ...
        if (isset($_conifg['session'][$gc])
        && !empty($_conifg['session'][$gc])
        && is_integer($_conifg['session'][$gc]))
        {
            ini_set('session.'.$gc,$_conifg['session'][$gc]);
        }
    }
